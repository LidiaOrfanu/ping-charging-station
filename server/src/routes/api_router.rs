...
use crate::{
    handlers::{
        charging_station::{
            handle_get_all_stations, handle_post_a_station,
...
pub fn create_api_locations_router(app_state: Arc<AppState>) -> Router {
    Router::new()
        .route("/locations", 
                get(handler_get_all_locations)
                .post(handler_post_a_location))
        .route(
            "/location/:id",
...
pub fn create_api_stations_router(app_state: Arc<AppState>) -> Router {
    Router::new()
        .route("/stations", 
            get(handle_get_all_stations)
            .post(handle_post_a_station))
        .route(
            "/station/:id",
